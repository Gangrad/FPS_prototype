...
namespace CsExtensions {
    public static class CollectionUtils {
        ...
        public static IEnumerator<T> GetTypedEnumerator<T>(this IReadOnlyList<T> array) {
            for (int i = 0, count = array.Count; i < count; ++i)
                yield return array[i];
        }

        public static T GetByIdOrNull<T>(this IReadOnlyList<T> list, int id) where T : class {
